<template>
    <h3 class="text-h3">Objetivo Estratégico Institucional</h3>
    <div class="">
      <q-form class="row items-center q-col-gutter-md">
        <!-- buscador y botones -->
        <q-input outlined v-model="searchText" placeholder="Buscar..." dense clearable @clear="clearSearch"
          @keydown.enter="performSearch" class="col-8">
          <template v-slot:append>
            <q-icon name="search" class="cursor-pointer" @click="performSearch" />
          </template>
        </q-input>
        <div class="col-4 q-px-sm justify-end">
          <q-btn label="crear" color="primary" @click="crearPrograma"></q-btn>
          <q-btn label="ponderar"></q-btn>
        </div>
      </q-form>
      <!-- tabla -->
      <tablaProgramas />
